...
MAX_PARTICIPANTS = 1  # Лимит на количество участников тренировки
    ...
    if pending_data and pending_data['user_id'] == user.id:
        ...
        if text == 'да':
            ...
            if user_participant and user_participant['status'] != 'reserve':
                ...
                # Оповещаем всех пользователей об изменении
                await notify_all_users_about_update(context, event_id, user.id, user.username, "отписался")
                await update.message.reply_text("Обновлено", reply_markup=create_static_keyboard(event_id, user.id))
...
async def notify_all_users_about_update(context: ContextTypes.DEFAULT_TYPE, event_id, userId, username, action):
...
async def join_event(update: Update, context: ContextTypes.DEFAULT_TYPE, event_id, user):
    ...
    if len(participants) < MAX_PARTICIPANTS:
        participants.append({'user_id': user.id, 'username': user.username, 'status': 'confirmed'})
        await update.message.reply_text(f"Вы записаны под номером {len(participants)}.\nДобро пожаловать в основной состав!🤝")
    else:
        participants.append({'user_id': user.id, 'username': user.username, 'status': 'reserve'})
    ...
    # Оповещаем всех пользователей об изменении
    await notify_all_users_about_update(context, event_id, user.id, user.username, "записался")
    await update.message.reply_text("Обновлено", reply_markup = create_static_keyboard(event_id, user.id))
...
if __name__ == "__main__":
    ...
    # Планируем создание событий по вторникам и четвергам
    application.job_queue.run_repeating(schedule_events, interval=86400, first=0)
